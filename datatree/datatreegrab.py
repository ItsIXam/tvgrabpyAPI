...
class _Warnings():
    ...
    def _show_warning(self, message, category, caller_id, severity, lineno):
        with self.warn_lock:
            message = "%s,id:%s:%s at line:%s: %s\n" % (MODULE_NAME, caller_id, category.name, lineno, message)
            try:
...
class DataDef_Convert():
    def __init__(self, data_def = None, warnaction = "default", warngoal = sys.stderr, caller_id = 0):
        ...
        with self.tree_lock:
            ...
            self.cdata_def = {}
            self.ddtype = ""
            if sys.modules[MODULE_NAME]._warnings == None:
                sys.modules[MODULE_NAME]._warnings = _Warnings(warnaction, warngoal, caller_id)

            elif caller_id not in sys.modules[MODULE_NAME]._warnings._ids or warnaction != None:
                sys.modules[MODULE_NAME]._warnings.set_warnaction(warnaction, caller_id)
    ...
    def simplefilter(self, action, category=Warning, lineno=0, append=0, severity=0):
        with self.tree_lock:
            sys.modules[MODULE_NAME]._warnings.simplefilter(action, category, lineno, append, self.caller_id, severity)

    def warn(self, message, category, severity, stacklevel = 2):
        sys.modules[MODULE_NAME]._warnings.warn(message, category, self.caller_id, severity, stacklevel)
